...
if [ $LOG_STDERR ]; then
    /bin/ln -sf /proc/self/fd/1 /var/log/apache2/access.log && \
    /bin/ln -sf /proc/self/fd/1 /var/log/apache2/error.log
else
	LOG_STDERR='No.'
fi
...
cat << EOB
    ...
    *    This host targets    : $MYSQL_PORT 
    *    Docker image: markchristopherwest/docker-lamp               *
    *    https://github.com/markchristopherwest/docker-lamp   *
    *                                            *
    *************************************************************************
    SERVER SETTINGS
    ---------------
    · Redirect Apache access_log to STDOUT [LOG_STDOUT]: $LOG_STDOUT
...
# Set PHP timezone
/bin/sed -i "s/\;date\.timezone\ \=/date\.timezone\ \=\ ${DATE_TIMEZONE}/" /etc/php/8.0/apache2/php.ini
